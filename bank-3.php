...
#try{
#$pdo->select_db(K_USERNAME);
#}
#catch (PDOException $e) {
#    error_log('2PDO Exception: '.$e->getMessage());
#    die('PDO says no.');
#}
...
function getchecking(){
	$.post("bank-3.php",{"getchecking":1}, function(data, status){
	      $("#checker").text(data);
	    });
	return parseFloat($("#checker").text());
}
function getsavings(){
	$.post("bank-3.php",{"getsavings":1}, function(data, status){
	      $("#saver").text(data);
	    });
...
$(document).ready(function(){
   ...
   $(this).keypress(function (e) {
	...
	$("#checker").click(function(){
	    $.post("bank-3.php",{"depchecking":0.01}, function(data, status){
	 ...
	 $("#saver").click(function(){
	 	    $.post("bank-3.php",{"depsavings":0.01}, function(data, status){
	 			if(data.toString().search("error") != -1)
	 	      		alert("error!" + data.toString());
	 	      	getsavings();
	 	    });
  ...
  $("#depchecks").click(function(){
    ...
    $.post("bank-3.php",{"depchecking":text}, function(data, status){
		if(data.toString().search("error") != -1)
      		alert("error!" + data.toString());
      	getchecking();
    });
  ...
  $("#depsav").click(function(){
      ...
      $.post("bank-3.php",{"depsavings":text}, function(data, status){
  		if(data.toString().search("error") != -1)
  		  alert("error!" + data.toString());
  		  getsavings();
      });
...
$("#tchecks").click(function(){
    ...
    $.post("bank-3.php",{"tchecking":text}, function(data, status){
		if(data.toString().search("error") != -1)
      		alert("error!" + data.toString());
      	getchecking();
      	getsavings();
  ...
  $("#tsavs").click(function(){
      ...
      $.post("bank-3.php",{"tsavings":text}, function(data, status){
  		if(data.toString().search("error") != -1)
        		alert("error!" + data.toString());
        getchecking();
       	getsavings();
	...
	$("#fresh").click(function(){
	    $.post("bank-3.php",{"fstart":1}, function(data, status){
	      //alert("Data: " + data + "\nStatus: " + status);
	    });
	    getchecking();
	    getsavings();
	...
	$("#nook").click(function(){
		    $.post("bank-3.php",{"nook":1}, function(data, status){
		      //alert("Data: " + data + "\nStatus: " + status);
		    });
		    getchecking();
		    getsavings();
...
$("#crook").click(function(){
	    $.post("bank-3.php",{"crook":1}, function(data, status){
	      //alert("Data: " + data + "\nStatus: " + status);
	    });
	    getchecking();
	    getsavings();
   ...
   <li>Deposit into checking <input id = "depcheckstext" type = "text" name = "depchecking" /> <button id="depchecks">submit</button></li>
   <li>Deposit into savings <input id = "depsavstext" type = "text" name = "depsavings" /> <button id="depsav">submit</button></li>
   <li>Transfer from checking into savings <input id="tcheckstext" type = "text" name = "tchecking" /> <button id="tchecks">submit</button></li>
   <li>Transfer from savings into checking <input id="tsavstext" type = "text" name = "tsavings" /> <button id="tsavs">submit</button></li>
